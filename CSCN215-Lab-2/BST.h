...
private:
    ...
    BSTNode<Key, E>* getmin(BSTNode<Key, E>*) const;
...
public:
    ...
    void printInorder() const {
        if (root == NULL) cout << "The BST is empty.\n";
            ...
            BSTNode<Key, E>* current = getmin(root);
            while (current != NULL) {
    ...
    void printReverse() const {
        if (root == NULL) cout << "The BST is empty.\n";
            ...
            while (current->isThreadedRight() != 0) {
...
BSTNode<Key, E>* BST<Key, E>::inserthelp(
    ...
    if (k < root->key()) {
        if (root->isThreadedLeft()) {
            // Inserting as left child with threading
            BSTNode<Key, E>* newNode = new BSTNode<Key, E>(k, it, root->left(), root);
            root->setThreadedLeft(false);
        ...
        if (root->isThreadedLeft()) {
            // Inserting as right child without threading
            root->setThreadedLeft(false);
            BSTNode<Key, E>* newNode = new BSTNode<Key, E>(k, it, root, root->right());
...
template <typename Key, typename E>
BSTNode<Key, E>* BST<Key, E>::getmin(BSTNode<Key, E>* rt) const {
    if (rt == NULL)
        return NULL;
...
removehelp(BSTNode<Key, E>* rt, const Key& k) {
    if (rt == NULL) return NULL;    // k is not in the tree
        ...
        if (!rt->isThreadedLeft()) {
            // Node has a right child, so traverse normally
